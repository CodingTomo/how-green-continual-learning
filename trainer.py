...
def _train(args):
    ...
    method_emission_tracker = EmissionsTracker(log_level="critical", project_name="Method_{}".format(args["model_name"]), output_file=additional_logger_name+"{}_total_emissions.csv".format(args["model_name"]))
    ...
    with open(additional_logger_name + "average_incremental_accuracy.txt", "w") as file:
        for item in average_incremental_accuracy:
            file.write("%s\n" % item)
    
    # Save in file the per step incremental accuracy
    with open(additional_logger_name + "per_step_incremental_accuracy.txt", "w") as file:
        for item in per_step_incremental_accuracy:
            file.write("%s\n" % item)

    # Save in file all results
    with open(additional_logger_name + "all_results.txt", "w") as file:
        for item in all_results:
            file.write("%s\n" % item)

    if args["only_inference"] == "n":
        #inference on final task
        model.inference_gpu_time(args["model_name"], additional_logger_name)
