...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        task_emission_tracker = EmissionsTracker(log_level="critical", project_name="EASE_Task_{}".format(self._cur_task), output_file=self.outpath+"EASE_per_task_emissions.csv")
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            losses = 0.0
            correct, total = 0, 0
            epoch_emission_tracker = EmissionsTracker(log_level="critical", project_name="EASE_Task_{}_Epoch_{}".format(self._cur_task,epoch), output_file=self.outpath+"EASE_per_epoch_emissions.csv")
    ...
    def inference_gpu_time(self, model_name, outpath):
        ...
        with torch.no_grad():
            inference_tracker = EmissionsTracker(log_level="critical", project_name="EASE_inference_Task_{}".format(self._cur_task), output_file=self.outpath+"EASE_per_task_inference_emissions.csv")
            inference_tracker.start()
            for rep in range(repetitions):
                starter.record()
                self._network(dummy_input, test=True)
                ender.record()
