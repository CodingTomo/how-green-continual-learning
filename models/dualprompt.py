...
class Learner(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task > 0 and self.args["reinit_optimizer"]:
            optimizer = self.get_optimizer()
        task_emission_tracker = EmissionsTracker(log_level="critical", project_name="DUALP_Task_{}".format(self._cur_task), output_file=self.outpath+"DUALP_per_task_emissions.csv")
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            losses = 0.0
            correct, total = 0, 0
            epoch_emission_tracker = EmissionsTracker(log_level="critical", project_name="DUALP_Task_{}_Epoch_{}".format(self._cur_task,epoch), output_file=self.outpath+"DUALP_per_epoch_emissions.csv")
    ...
    def inference_gpu_time(self, model_name, outpath):
        ...
        with torch.no_grad():
            inference_tracker = EmissionsTracker(log_level="critical", project_name="DUALP_inference_Task_{}".format(self._cur_task), output_file=self.outpath+"DUALP_per_task_inference_emissions.csv")
