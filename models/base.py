...
class BaseLearner(object):
    ...
    def inference_gpu_time(self, model_name, outpath):
        ...
        with torch.no_grad():
            inference_tracker = EmissionsTracker(log_level="critical", project_name="Method_{}".format(model_name), output_file=outpath+"{}_gpu_inference_emissions.csv".format(model_name))
    ...
    def inference_cpu_time(self, model_name, outpath):
        ...
        with torch.no_grad():
            inference_tracker = EmissionsTracker(log_level="critical", project_name="Method_{}".format(model_name), output_file=outpath+"{}_cpu_inference_emissions.csv".format(model_name))
            inference_tracker.start()
            for rep in range(repetitions):
